...
const mockClientAdmin: ClientAdmin = {
  id: "client-admin-1",
  firstName: "Maria",
  lastName: "Rodriguez",
...
describe("useClientAdminForm", () => {
  ...
  describe("form initialization", () => {
    it("initializes with client admin data", () => {
      const { result } = renderHook(
        () => useClientAdminForm(mockClientAdmin, true, mockOnSuccess),
  ...
  describe("form submission", () => {
    it("only sends changed fields", async () => {
      ...
      server.use(
        http.patch("*/client-admins/client-admin-1", async ({ request }) => {
          ...
          return HttpResponse.json(mockClientAdmin);
        })
      );
      ...
      await act(async () => {
        await result.current.onSubmit({
          ...
          lastName: mockClientAdmin.lastName,
          email: mockClientAdmin.email,
          phone: mockClientAdmin.phone,
          jobTitle: mockClientAdmin.jobTitle,
          isActive: mockClientAdmin.isActive,
        });
      });
    ...
    it("shows toast for no changes", async () => {
      ...
      await act(async () => {
        await result.current.onSubmit({
          firstName: mockClientAdmin.firstName,
          lastName: mockClientAdmin.lastName,
          email: mockClientAdmin.email,
          phone: mockClientAdmin.phone,
          jobTitle: mockClientAdmin.jobTitle,
...
describe("useClientAdminDetail", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("fetches client admin data", async () => {
    server.use(
    ...
    const { result } = renderHook(() => useClientAdminDetail("client-admin-1"), {
      wrapper: createWrapper(),
    });
    ...
    expect(result.current.clientAdmin).toEqual(mockClientAdmin);
  });
  ...
  it("returns isError on failure", async () => {
    ...
    expect(result.current.clientAdmin).toBeUndefined();
